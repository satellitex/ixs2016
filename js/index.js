$(document).ready(function() {
	...
	$("button.code").click(function(){
		...
		function solve(input){    
		    ...
		    var want = new Array(N); // i番目の素材が何μlほしいか
		    result["kind"] = new Array(W);
		    result["fie"] = new Array(W);
		    result["need"] = new Array(W);
		    result["color"] = new Array(W);
		    for( var x=0;x<W;x++){
			    result["kind"] = new Array(H);
			    result["fie"] = new Array(H);
			    result["need"] = new Array(H);
			    result["color"] = new Array(H);
			    for( var y=0;y<H;y++ ){
			        result["kind"][x][y] = -1;
			        result["color"][x][y] = 0;
			    }
		    ...
		    //混合後を配置
		    cnt = K;
		    for( i=N-1;i>=des1+des2&&cnt>0;i-- ){
		    for( x=0;x<pour[cnt-1];x++){//ここバグあるよ
		        result.kind[i][y] = 2;
		        result.fie[i][y] = cnt-1;
		    }
		    cnt--;
		    }
		    //綺麗に置けない
		    ...
		    for( y=0;y<H;y++ ){
		        ...
		        result.color[x][y] = input.color[x][y];
		        } else {
		        for( i=0;i<M;i++ )
		    ...
		    for( y=0;y<H;y++ ){
		        ...
		        for( var x2=0;x2<W;x2++ ){
		            ...
		            if( plane.kind[x2][y2] == 2 ){
		                if( isIncludeSid( input.value, plane.fie[x][y] ) ){
		...
		function isIncludeSid( mix_v, sid ){
